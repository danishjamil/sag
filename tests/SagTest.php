...
class SagTest extends PHPUnit_Framework_TestCase
{
    ...
    {
        $result = $this->couch->post(['keys' => ['1']], '/_all_docs');
        $this->assertEquals(1, count($result->body->rows));
        $this->assertEquals('1', $result->body->rows[0]->id);

        $validData = new stdClass();
        $invalidPaths = [[], '', new stdClass(), false, true];
    ...
    public function test_queryEmptyView()
    {
        ...
        $this->assertEquals(count($result->body->rows), 0, 'Empty rows array');
    ...
    public function test_getAllDocs()
    {
        ...
        $this->assertEquals(count($resDefaults->body->rows),
      count($resAllWithDocs->body->rows));
        ...
        $this->assertEquals('1',
      $this->couch->getAllDocs(true, null, null, null, ['1'])->body->rows[0]->id);
    ...
    public function test_genIDs()
    {
        ...
        $this->assertEquals(count($uuids), 10);
    }
    ...
    public function test_bulk()
    {
        ...
        $this->assertEquals(count($result->body), count($docs));

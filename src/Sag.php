  ...
  private $cache;
    ...
    public function get($url)
    {
        ...
        if ($this->cache) {
            ...
            if ($prevResponse) {
                $response = $this->procPacket('GET', $url, null, ['If-None-Match' => $prevResponse->headers->etag]);
    ...
    public function setAttachment($name, $data, $contentType, $docID, $rev = null)
    {
        ...
        return $this->procPacket('PUT', "/{$this->db}/{$docID}/{$name}".(($rev) ? '?rev='.urlencode($rev) : ''), $data, ['Content-Type' => $contentType]);
    }
    ...
    private function procPacket($method, $url, $data = null, $headers = [])
    {
        /*
       ...
       */
        ...
        if (is_array($this->globalCookies) && count($this->globalCookies)) {
